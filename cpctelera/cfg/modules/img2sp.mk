...
#################
# IMG2SP_SET_PALETTE_PLUS: Sets the CPC Plus RGB palette to be used in following IMG2SP CONVERT commands
#
# $(1): RGB palette array (Array of hex colors 0x000000 to 0xFFFFFF, max size 16)
#
# Updates variable I2S_PAL
#
define IMG2SP_SET_PALETTE_PLUS
	# Verify passed RGB palette
	$(call VERIFY_RGB_PALETTE,$(1),IMG2SP - SET_PALETTE_PLUS)

	# All checks passed, add brackets and -rgbp option
	$(eval I2S_PAL := -rgbp { $(1) })
endef
...
#
# $(1): Pixel Format { sprites, zgtiles, screen, spritehardware }
#
# Updates variable I2S_FMT
#
define IMG2SP_SET_IMG_FORMAT
	# Convert selected format into options
	$(eval _VALID := zgtiles screen sprites spritehardware)
	$(eval _CNV := -z'-g -scr '' -sh)
...
#################
# IMG2SP_CONVERT_PALETTE_PLUS: Converts an RGB palette into .c/.h files with CPC Plus ASIC values (u16)
# It only converts the palette, without doing anything else
#
# $(1): RGB Palette Values (hex: 0xRRGGBB)
# $(2): Output C-identifier (it will also be used as filename)
#
# Updates IMGCFILES and OBJS2CLEAN adding new C files that result from the pack generation.
#
define IMG2SP_CONVERT_PALETTE_PLUS
	# Verify RGB palette before converting
	$(call VERIFY_RGB_PALETTE,$(1),CONVERT_PALETTE_PLUS)
	# Empty contents before filling it up with new ASIC values
	$(eval I2SCP_PASIC:=)
	# I2SCP_PASIC fills up with ASIC 16-bit values (0x0GRB) converted from RGB values
	$(call CONVERT_RGB2ASIC_PALETTE,$(1),I2SCP_PASIC)
	...
	# Check that it is a valid C-identifier
	$(call ENSURE_VALID_C_ID,$(I2SCP_CID),$(I2S_ERR) CONVERT_PALETTE_PLUS]: '$(I2SCP_CID)' is not a valid C-identifier. \
		As it will be used both for filename and array identifier, it needs to be a valid C-identifier)
...
.SECONDARY: $(I2SCP_FILES)
$(I2SCP_FILES): $(I2S_DEPEND)
	@$(call PRINT,$(PROJNAME),"Generating CPC Plus ASIC palette hardware values for '$(I2SCP_CID)'...")
	@echo "// File generated by CONVERT_PALETTE_PLUS macro from CPCtelera" > $(I2SCP_HFILE)
	@echo "// Converted CPC Plus ASIC palette declaration" >> $(I2SCP_HFILE)
	@echo "// " >> $(I2SCP_HFILE)
	@echo "#ifndef _$(basename $(I2SCP_CID))_H_" >> $(I2SCP_HFILE)
	@echo "#define _$(basename $(I2SCP_CID))_H_" >> $(I2SCP_HFILE)
	@echo "" >> $(I2SCP_HFILE)
	@echo "#include <types.h>" >> $(I2SCP_HFILE)
	@echo "" >> $(I2SCP_HFILE)
	@echo "extern const u16 $(I2SCP_CID)[$(I2SCP_PSZ)];" >> $(I2SCP_HFILE)
	@echo "" >> $(I2SCP_HFILE)
	@echo "#endif" >> $(I2SCP_HFILE)
	@echo "// File generated by CONVERT_PALETTE_PLUS macro from CPCtelera" > $(I2SCP_CFILE)
	@echo "// RGB Palette Converted: $(1)" >> $(I2SCP_CFILE)
	@echo "// " >> $(I2SCP_CFILE)
	@echo "#include \"$(notdir $(I2SCP_HFILE))\"" >> $(I2SCP_CFILE)
	@echo " " >> $(I2SCP_CFILE)
	@echo "const u16 $(I2SCP_CID)[$(I2SCP_PSZ)] = {" >> $(I2SCP_CFILE)
	@echo "   $(patsubst %$(COMMA)vvv,%,$(patsubst %,0x%$(COMMA),$(I2SCP_PASIC))vvv)" >> $(I2SCP_CFILE)
	@echo "};" >> $(I2SCP_CFILE)
...
define IMG2SP 
	...
	$(eval IMG2SP_F_FUNCTIONS = SET_PALETTE_FW SET_PALETTE_PLUS SET_MODE SET_IMG_FORMAT SET_MASK SET_OUTPUT SET_FOLDER SET_EXTRAPAR CONVERT CONVERT_PALETTE CONVERT_PALETTE_PLUS)
