...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Function: cpct_drawMaskedToSpriteBufferHFlipM1
;;
;; Draws a Mode 1 sprite masked from an array inside another sprite's buffer
;; flipping it Horizontally (right to left).
;; This permits using the destination sprite as a temporary screen back buffer.
;;
;; C Definition:
;;    void <cpct_drawMaskedToSpriteBufferHFlipM1> 
;;           (<u16> *buffer_width*, void* *inbuffer_ptr*, <u8> *width*, <u8> *height*, void* *sprite*) __z88dk_callee;
;;
...
;;    > call cpct_drawMaskedToSpriteBufferHFlipM1_asm
;;
...
;;  * *sprite* must be a pointer to an array containing sprite's pixels along with mask data
;; in screen pixel format. Sprite must be rectangular and all bytes in the array must 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   ld   a, c                     ;; [1] A = C = width
   dec  a                        ;; [1] A--
   add_de_a                      ;; [5] DE += width - 1 (Point to last byte at the buffer)

   jr  drawByte                  ;; [3] First byte does not require C to be restored nor DE/HL to be changed
  
;; Perform the copy
nextByte:
   inc  hl                            ;; [2] HL++ (Next sprite byte)
   dec  de                            ;; [2] DE-- (Destination previous byte in buffer memory. We draw right-to-left)

drawByte:
   push bc                            ;; [4] Save BC

   ld   a, (hl)                       ;; [2] A = Next sprite byte
   ld   c, a                          ;; [1] C = A for reverse
   cpctm_reverse_mode_1_pixels_of_A c ;; [7] Reverses both pixels of sprite byte in A
   ld   b, a                          ;; [1] B = A (Reversed mask)
   
   inc  hl                            ;; [2] HL += 1 => Point HL to Sprite Colour information
   ld   a, (hl)                       ;; [2] A = Next sprite byte
   ld   c, a                          ;; [1] C = A for reverse
   cpctm_reverse_mode_1_pixels_of_A c ;; [7] Reverses both pixels of sprite byte in A
   ld   c, a                          ;; [1] C = A (Reversed color)
   
   ld   a, (de)                       ;; [2] Get next background byte into A
   and  b                             ;; [1] Erase background part that is to be overwritten (Mask step 1)
   or   c                             ;; [1] Add up background and sprite information in one byte (Mask step 2)
