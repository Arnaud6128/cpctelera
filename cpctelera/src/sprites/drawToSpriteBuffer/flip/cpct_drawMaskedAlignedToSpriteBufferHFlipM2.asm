...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Function: cpct_drawMaskedAlignedToSpriteBufferHFlipM2
;;
;; Draws a Mode 1 sprite masked aligned from an array inside another sprite's buffer
;; flipping it Horizontally (right to left).
;; This permits using the destination sprite as a temporary screen back buffer.
;;
;; C Definition:
;;    void <cpct_drawMaskedAlignedToSpriteBufferHFlipM2> 
;;           (<u16> *buffer_width*, void* *inbuffer_ptr*, <u8> *width*, <u8> *height*, void* *sprite*, <u8>* mask_table) __z88dk_callee;
;;
...
;;    > call cpct_drawMaskedAlignedToSpriteBufferHFlipM2_asm
;;
...
;;  * *mask_table* must be a pointer to the mask table that will be used for calculating
...
firstByte:
   ...
   cpctm_reverse_mode_2_pixels_of_A b ;; [7] Reverses both pixels of sprite byte in A

   ld   b, (hl)                   ;; [2] B = destination pixel
   push hl                        ;; [4] Store HL (Destination)
   
maskTable = .+1
   ld   hl, #0000                ;; [3] HL = Mask table
   ld   l, a                     ;; [1] Access mask table element (table must be 256-byte aligned)
   ld   a, b                     ;; [2] Get the value of the byte of the screen where we are going to draw
   ...
   pop  hl                       ;; [3] Restore HL (Destination)
