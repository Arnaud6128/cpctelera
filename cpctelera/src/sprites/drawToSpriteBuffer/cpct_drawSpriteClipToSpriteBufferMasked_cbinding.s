...
;; C bindings for <cpct_drawSpriteClipToSpriteBufferMasked>
;;
;;   28 us, 20 bytes
;;
_cpct_drawSpriteClipToSpriteBufferMasked::
   ;; Get parameters from A and DE registers ((8 + 16 bits) + (8 + 8 + 16 + 16 bits), with __sdcccall(1) convention
   ...
   neg                             ;; [1] A = -A
   add  a                          ;; [1] A = A * 2 (Sprite masked)
   ld  (spriteClipped_offset), a   ;; [4] Set Sprite Offset at placeholder  
   
   pop  bc                         ;; [3] B = Useles / C = Height 
   ex  (sp), hl                    ;; [5] HL = Source Sprite <-> (SP) Returning back address because __z88dk_callee convention     
      
.include /cpct_drawSpriteClipToSpriteBufferMasked.asm/    
