...
_cpct_drawSpriteClippedMaskedAlignedTable::
   ...
   ld  (save_iy), iy             ;; [5] Save IY

   ;; Compute next pixel line Lsb (0x0800 - width_to_draw = 0x07LL)
   xor  a                        ;; [1] A = 0
   sub  l                        ;; [1] A = A - L
   ld  (offset_to_next_line), a  ;; [4] Store A to offset for next line placeholder

   ld   a, l                     ;; [1] A = L (width_to_draw)
   ld  (width_to_draw), a        ;; [4] Store A Sprite width_to_draw at placeholder     
   
   ;; Get next parameters from stack
   pop  ix                       ;; [3] IX = Return Address
   pop  bc                       ;; [3] BC = Height/Width (B = Height, C = Width)
   pop  hl                       ;; [3] HL = Sprite pointer
   ex  (sp), ix                  ;; [5] IX = Mask table <-> (SP) = Returning back address because __z88dk_callee convention 
