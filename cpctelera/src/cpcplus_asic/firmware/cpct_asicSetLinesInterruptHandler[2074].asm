...
;; 17 + 30 + 23 = 70
   ...
   ld (remaining_raslines_counter), a  ;; [4] And store the same value in the counter to initiate it with the total number of raster lines (it counts downwards)
...
cpct_asicSafeInterruptHandlerHook:
   ...
   ;; if it is a C-function. Otherwise it will fail to retrieve parameter A
   ;;
next_rasline = .+1
   ld    a, #byte_ph          ;; [2] A = Next rasterline number
safeInterruptHandlerCall = .+1   
   call  #word_ph             ;; [5] Call Interrupt Handler (_z88dk_fastcall, it expects 8-bit parameter in A)

   ;;
   ;; Set up next raster interrupt line
   ;;
remaining_raslines_counter = .+1
   ld    hl, #remaining_raslines_counter  ;; [3] HL Points to the remaining raster lines counter variable
   ...
   ;; A!=0. Thare are still some raster line interrupts to be delivered
...
total_raslines = .+1
   ...
   ld  (remaining_raslines_counter), a ;; [4] Save this number for next interrupt call
raslines_array_ptr = .+1
   ld    hl, #word_ph                  ;; [3] HL = Pointer to the start of the raster lines array
   ld  (ptr_next_raster_line_val), hl  ;; [5] Save this pointer for next interrupt call
   ld     a, (hl)                      ;; [2] A = first raster line number
