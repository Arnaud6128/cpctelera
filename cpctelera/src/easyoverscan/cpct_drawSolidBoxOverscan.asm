...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Function: cpct_drawSolidBoxOverscan
;;
;;    Fills up a rectangle in video memory overscan(or screen buffer) with a given 
;; colour data byte. Could be used for drawing coloured rectangles as well as
;; erasing screen rectangles easily.
;;
;; C Definition:
;;    void <cpct_drawSolidBoxOverscan> (void* *memory*, <u8> *colour_pattern*, <u8> *width*, <u8> *height*) __z88dk_callee;
;;
;; Input Parameters (5 bytes):
;;  (2B DE) memory         - Video memory pointer to the upper left box corner byte
;;  (2B HL) colour_pattern - L = 1-byte colour pattern (in screen pixel format) to fill the box with
;;  (1B C ) width          - Box width *in bytes* [1-64] (Beware! *not* in pixels!)
;;  (1B B ) height         - Box height in bytes (>0)
;;
;; Assembly call (Input parameters on registers):
;;    > call cpct_drawSolidBoxOverscan_asm
;;
...
;; DrawSolidBoxOverscan code starts here (immediately after getting parameters from stack)
...
check_boundaries:
   ...
   ;; reposition destination pointer. That means our next line is 16K-0x60 bytes back
   ;; which is the same as advancing 48K+0x60 = 0xC060 bytes, as memory is 64K 
   ;; and our 16bit pointers cycle over it
   ld    hl, #0xC060       ;; [3] We advance destination pointer to next line
   add   hl, de            ;; [3] HL = DE + 0xC060
   ld     d, h             ;; [1] | DE = HL
   ld     e, l             ;; [1] |
...
next_bank_line:    
   ...
   ld    e, a              ;; [1] Update E
   ld    l, e              ;; [1] L = E
   jp    nc, _next_line    ;; [3] Continue if no carry to H
   inc   d                 ;; [1] Increment H to sync bank address
   ld    h, d              ;; [1] H = D
   jp    _next_line        ;; [3] Continue drawing next line
