;       AKY Multi-PSG music player - V1.1.
;       By Julien Névo a.k.a. Targhan/Arkos.
;       CPC PSG sending optimization trick by Madram/Overlanders.
;       2019-2025.
;
;       v1.3: Added PCW support / PCW 6-channel Turbosound support.
;       v1.2: Added digidrums and sample support.
...
;
;       This compiles with RASM. Please check the compatibility page on Arkos Tracker website, there is a Source Converter (Disark) for ANY assembler!
;
...
;       PLY_AKY_ROM = 1
;       PLY_AKY_ROM_Buffer = #4000 (or wherever). The buffer is PLY_AKY_ROM_BufferSize long (42 bytes, or 81 if using sound effects).
;       This makes the player a tiny bit slower.
;
;       Digidrums
;       ----------------------
;       Digidrums are supported on one channel:
;       PLY_AKY_USE_DIGIDRUMS = 1
;       PLY_AKY_DIGIDRUM_PSG_NUMBER = 1 (optional, 1 is default).
;       See the tester on how to initialize them. Samples and events must be exported from AT and included.
;
;       Samples
;       ----------------------
;       Samples are supported on all three channels, but one sample at a time.
;       PLY_AKY_USE_SAMPLE = 1
;       See the tester on how to initialize them. Samples and sample pattern must be exported from AT and included.
;
;       Optimizations
;       ----------------------
...
PLY_AKY_OPCODE_SCF equ #ff                         ;255 * 2 = carry.
        ...
        ;Simpler to declare this even if digidrums are not used.
        IFNDEF PLY_AKY_DIGIDRUM_PSG_NUMBER
                PLY_AKY_DIGIDRUM_PSG_NUMBER = 1               ;>=1. A default value.
        ENDIF

        IFDEF PLY_AKY_USE_DIGIDRUMS
                PLY_AKY_CAN_OVERRIDE_VOLUMES = 1

                IF PLY_AKY_DIGIDRUM_PSG_NUMBER <= 0
                        FAIL 'PLY_AKY_DIGIDRUM_PSG_NUMBER must be >=1!'
                ENDIF
        ENDIF

        IFDEF PLY_AKY_USE_SAMPLE
                PLY_AKY_CAN_OVERRIDE_VOLUMES = 1

                IFNDEF PLY_AKY_SAMPLE_PSG_NUMBER
                        PLY_AKY_SAMPLE_PSG_NUMBER = 1                 ;>=1. A default value.
                ENDIF

                IF PLY_AKY_SAMPLE_PSG_NUMBER <= 0
                        FAIL 'PLY_AKY_DIGIDRUM_PSG_NUMBER must be >=1!'
                ENDIF
        ENDIF
...
PLY_AKY_HardwareCounter = 0
        ...
        IFDEF PLY_AKY_HARDWARE_MSX      ;Important to test it BEFORE the MSX related hardware, because the same constant is used!
                ...
                PLY_AKY_HARDWARE_MSX_VANILLA = 1 ;A special variable is used to make the difference with multi-PSG hardware.
        ...
        IFDEF PLY_AKY_HARDWARE_DARKY
                ...
                PLY_AKY_PsgCount = 2
        ENDIF
        IFDEF PLY_AKY_HARDWARE_PCW_TURBOSOUND
                PLY_AKY_HardwareCounter = PLY_AKY_HardwareCounter + 1
                PLY_AKY_HARDWARE_FAST_PSG_ACCESS = 1
                PLY_AKY_PsgCount = 2
        ENDIF
...
PLY_AKY_PCW_TURBOSOUND_PSGNUMBER1_SELECTION_VALUE equ #ff  ;Value to select the PSG1 later on, for the PCW TurboSound.
PLY_AKY_PCW_TURBOSOUND_PSGNUMBER2_SELECTION_VALUE equ #fe  ;Value to select the PSG2 later on, for the PCW TurboSound.
...
PLY_AKY_Channel{channelNumber}_RegisterBlock_Return:
        ...
        ;If digidrums, calls the "manage drums" method to set the R7 bit or not, and to set-up the sample pointer.
        IFDEF PLY_AKY_USE_DIGIDRUMS
                IFDEF PLY_AKY_USE_SAMPLE
                        FAIL 'Use either sample or digidrums, but not both!'
                ENDIF
                IF psgNumber == PLY_AKY_DIGIDRUM_PSG_NUMBER
                        jp PLY_AKY_ManageDigidrums
PLY_AKY_ManageDigidrums_Return
                ENDIF
        ENDIF
        ;If sample, calls the "manage sample" method to set the R7 bit or not, and to set-up the sample pointer.
        IFDEF PLY_AKY_USE_SAMPLE
                IFDEF PLY_AKY_USE_DIGIDRUMS
                        FAIL 'Use either sample or digidrums, but not both!'
                ENDIF
                IF psgNumber == PLY_AKY_SAMPLE_PSG_NUMBER
                        jp PLY_AKY_ManageSample
PLY_AKY_ManageSample_Return
                ENDIF
        ENDIF
...
PLY_AKY_SaveSpTemp: ld sp,0
        ...
        IFDEF PLY_AKY_HARDWARE_CPC             ;Nothing to do for CPC, the code doesn't need any parameter.
                PLY_AKY_PSG_SENDING_SETUP = 1
        ENDIF
        ...
        IFDEF PLY_AKY_HARDWARE_PCW_TURBOSOUND
                ...
                IF psgNumber == 1
                        ld a,PLY_AKY_PCW_TURBOSOUND_PSGNUMBER1_SELECTION_VALUE
                ELSEIF psgNumber == 2
                        ld a,PLY_AKY_PCW_TURBOSOUND_PSGNUMBER2_SELECTION_VALUE
                ELSE
                        FAIL("Illegal PSG number. Only 1 or 2 is acceptable.")
                ENDIF
                out (#aa),a      ;Register.
        ...
        ;Security.
        IFNDEF PLY_AKY_PSG_SENDING_SETUP
                FAIL "You didn't set up the PSG sending!"       ;See above for how it is done.
        ENDIF
...
PLY_AKY_Psg{psgNumber}AfterSendingPsgRegisters:
        ...
        ;If digidrums, plays a frame of sample, if any.
        IFDEF PLY_AKY_USE_DIGIDRUMS
                IF psgNumber == PLY_AKY_DIGIDRUM_PSG_NUMBER
                        jp PLY_AKY_PlayDigidrums
PLY_AKY_PlayDigidrums_Return
                ENDIF
        ENDIF

        ;If sample, plays a frame of sample, if any.
        IFDEF PLY_AKY_USE_SAMPLE
                IF psgNumber == PLY_AKY_SAMPLE_PSG_NUMBER
                        jp PLY_AKY_PlaySample
PLY_AKY_PlaySample_Return
                ENDIF
        ENDIF
...
;Register 8
        IFDEF PLY_AKY_CAN_OVERRIDE_VOLUMES
            IFNDEF PLY_AKY_ROM
PLY_AKY_RegisterToSkip ld a,0           ;Now that A has been used just above, we can overwrite it.
            ELSE
            ld a,(PLY_AKY_RegisterToSkip)
            ENDIF
                ;While playing a digidrum, the player must not use its own value, else the sound cracks.
                cp 8
                jr nz,PLY_AKY_Register8
                inc hl
                jr PLY_AKY_Register8End
        ENDIF
PLY_AKY_Register8
        out (c),c
        ld b,e
...
;Register 9
        IFDEF PLY_AKY_CAN_OVERRIDE_VOLUMES
                cp 9
                jr nz,PLY_AKY_Register9
                inc hl
                jr PLY_AKY_Register9End
        ENDIF
PLY_AKY_Register9
        out (c),c
        ld b,e
...
;Register 10
        IFDEF PLY_AKY_CAN_OVERRIDE_VOLUMES
                cp 10
                jr nz,PLY_AKY_Register10
                inc hl
                jr PLY_AKY_Register10End
        ENDIF
PLY_AKY_Register10
        out (c),c
        ld b,e
...
PLY_AKY_PsgPlayCityRegister13_End:
        ...
        ;Used by Vanilla Spectrum, or SpecNext or Turbosound.
...
PLY_AKY_Register8End
        ...
        IFDEF PLY_AKY_CAN_OVERRIDE_VOLUMES
                ;While playing a digidrum, the player must not use its own value, else the sound cracks.
                ld a,(PLY_AKY_RegisterToSkip + PLY_AKY_Offset1b)
                cp 9
                jr nz,PLY_AKY_Register9
                inc hl
                jr PLY_AKY_Register9End
PLY_AKY_Register9 ld a,9
        ELSE
                inc a
        ENDIF
        out (c),a
...
PLY_AKY_PsgSpectrumRelatedRegister13_End
        ...
        ;The PSG Number is useful to know if the digidrum labels must be generated.
        MACRO PLY_AKY_GenerateSendPsgRegistersFast hexPortRegister,hexPortValue,psgNumber

;Sends the registers to one of the PSG, for lucky hardware with simple and fast OUTs (register/value).
;IN:    B = Register7 value.
;       HL = Register list, starting at register 0.
PLY_AKY_SendPsgRegisters_FastPsgAccess_{hexPortRegister}_{hexPortValue}
        ;Register 7.
        ld a,7
...
PLY_AKY_Register10End
        ...
        cp (hl)                         ;If IsRetrig?, force the R13 to be triggered.
                        IFDEF PLY_AKY_USE_Noise         ;CONFIG SPECIFIC
        jr z,PLY_AKY_SendPsgRegisters_FastPsgAccess_{hexPortRegister}_{hexPortValue}_Register13_End
                        ELSE
        ret z
                        ENDIF ;PLY_AKY_USE_Noise
        ld (hl),a                       ;The R13 becomes the "retrig" value, so that it is not played again, unless Retrig value is modified.
...
PLY_AKY_SendPsgRegisters_FastPsgAccess_{hexPortRegister}_{hexPortValue}_Register13_End
                        ELSE
                              ;No hardware but maybe noise.
                              IFDEF PLY_AKY_USE_Noise         ;CONFIG SPECIFIC
                                inc hl          ;Skips R11/R12/R13 data.
        ...
        IFDEF PLY_AKY_HARDWARE_FPGAPSG
                PLY_AKY_GenerateSendPsgRegistersFast a0,a1,1
                PLY_AKY_GenerateSendPsgRegistersFast 10,11,2
        ENDIF
        
        IFDEF PLY_AKY_HARDWARE_DARKY
                PLY_AKY_GenerateSendPsgRegistersFast 44,45,1
                PLY_AKY_GenerateSendPsgRegistersFast 4c,4d,2
        ENDIF
        ...
        IFDEF PLY_AKY_HARDWARE_PCW
                PLY_AKY_GenerateSendPsgRegistersFast aa,ab,1
        ENDIF

        IFDEF PLY_AKY_HARDWARE_PCW_TURBOSOUND
                ;There are two PSGs, but the same code is used.
                ;However, this prevents the digidrums from working well as the PSG number is the same
                ;for both PSG, so the digidrums would be played on both PSGs.
                ;This would need patching, but I won't do that unless asked to
                ;(the patch could be only adding the PSG number in the name of the generated code, pretty easy).
                PLY_AKY_GenerateSendPsgRegistersFast aa,ab,1
        ENDIF


        ENDIF ;PLY_AKY_HARDWARE_FAST_PSG_ACCESS
...
;------------------------------------------------------
        ...
        ENDIF
        IFDEF PLY_AKY_HARDWARE_FPGAPSG
                PLY_AKY_PSG1PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_a0_a1
                PLY_AKY_PSG2PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_10_11

                IF PLY_AKY_DIGIDRUM_PSG_NUMBER == 1
                        PLY_AKY_FAST_PSG_SELECT_REGISTER = #a0
                        PLY_AKY_FAST_PSG_SELECT_VALUE = #a1
                ELSEIF PLY_AKY_DIGIDRUM_PSG_NUMBER == 2
                        PLY_AKY_FAST_PSG_SELECT_REGISTER = #10
                        PLY_AKY_FAST_PSG_SELECT_VALUE = #11
                ELSE
                        FAIL 'Invalid PLY_AKY_DIGIDRUM_PSG_NUMBER!'
                ENDIF
        ENDIF
        IFDEF PLY_AKY_HARDWARE_DARKY
                PLY_AKY_PSG1PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_44_45
                PLY_AKY_PSG2PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_4c_4d

                IF PLY_AKY_DIGIDRUM_PSG_NUMBER == 1
                        PLY_AKY_FAST_PSG_SELECT_REGISTER = #44
                        PLY_AKY_FAST_PSG_SELECT_VALUE = #45
                ENDIF
                IF PLY_AKY_DIGIDRUM_PSG_NUMBER == 2
                        PLY_AKY_FAST_PSG_SELECT_REGISTER = #4c
                        PLY_AKY_FAST_PSG_SELECT_VALUE = #4d
                ENDIF
        ENDIF
        ...
        ENDIF
        IFDEF PLY_AKY_HARDWARE_PCW
                PLY_AKY_PSG1PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_aa_ab
        ...
        IFDEF PLY_AKY_HARDWARE_PCW_TURBOSOUND
                PLY_AKY_PSG1PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_aa_ab
                PLY_AKY_PSG2PLAYCODE = PLY_AKY_SendPsgRegisters_FastPsgAccess_aa_ab

                IFDEF PLY_AKY_USE_DIGIDRUMS
                        FAIL 'Digidrums are not available right now.' ;See above (PSG generation) about the reason.
                ENDIF
        ENDIF
        IFDEF PLY_AKY_HARDWARE_SPECTRUM_OR_PENTAGON
                PLY_AKY_PSG1PLAYCODE = PLY_AKY_SendPsgRegisters_SpectrumRelated
...
PLY_AKY_Channel{channelNumber}_SoundEffectSpeed                 equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 1
        ...
        IFDEF PLY_AKY_USE_DIGIDRUMS
PLY_AKY_RegisterToSkip                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 1
PLY_AKY_SampleVolumeRegister                                    equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 1
PLY_AKY_PT_SampleTable                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_ReadEventsWait                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_PT_Events                                               equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_PT_Sample                                               equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_PT_SampleEnd                                            equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_R7CloseChannelOpcode    set 1,b                 ;Modified.
        jp PLY_AKY_R7CloseChannelOpcode_Return
        ENDIF

        IFDEF PLY_AKY_USE_SAMPLE
PLY_AKY_RegisterToSkip                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 1
PLY_AKY_SPL_SampleVolumeRegister                                equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 1
PLY_AKY_SPL_ManageSampleWait                                    equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_PT_Pattern                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_PT_SampleTable                                      equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_Sample                                              equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_SampleEnd                                           equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_SampleLoop                                          equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_StepIntegerAndDecimal                               equ PLY_AKY_ROM_Buffer + PLY_AKY_ROM_BufferSize : PLY_AKY_ROM_BufferSize = PLY_AKY_ROM_BufferSize + 2
PLY_AKY_SPL_R7CloseChannelOpcode    set 1,b                 ;Modified.
        jp PLY_AKY_SPL_R7CloseChannelOpcode_Return
        ENDIF
...
PLY_AKY_Channel{channelNumber}_RegisterBase equ PLY_AKY_Psg{psgNumber}SoftwareRegisterArray + channelOffset
        ...
        IFDEF PLY_AKY_USE_DIGIDRUMS
                include "PlayerAkyMultiPsg_Digidrums.asm"
        ENDIF

        IFDEF PLY_AKY_USE_SAMPLE
                include "PlayerAkyMultiPsg_Sample.asm"
        ENDIF