...
//  Copyright (C) 2025 Arnaud Bouche (@Arnaud6128) for integration
//  Copyright (C) 2021 Néstor Gracia (https://github.com/nestornillo)
...
void main(void) {
  ...
  while (1) {
    ...
    // Clear the screen filling it up with 0's
    cpct_clearScreen_f64(0);
    // Decompress pack data_zx1 using ZX1 algorithm, so we need to
    // use as parameters for cpct_zx1_decrunch the start of the
    // destination area and the start of the compressed data
    cpct_zx1_decrunch (CPCT_VMEM_START, data_zx1);
    // Draw a string with the name of the decruncher function used
    cpct_drawStringM0("cpct_zx1_decrunch", pvmem);
    // Wait for a keyboard press
    do {cpct_scanKeyboard();} while (!cpct_isAnyKeyPressed());
